...
/* Live Preview конструктора етикетки */
(function ($) {
	'use strict';
	...
	function updateLabelPreview() {
		...
		// Співвідношення сторін для прев'ю
		var boxWidth = 240;
		...
		var fontMap = {
			small:  { ttn: '16px', name: '12px', text: '10px' },
			medium: { ttn: '19px', name: '14px', text: '11px' },
			large:  { ttn: '22px', name: '16px', text: '13px' }
		};
	...
	$(document).ready(function () {
		if ($('#nvx-label-template-card').length) {
			$('#nvx_lbl_width, #nvx_lbl_height, #nvx_lbl_m_top, #nvx_lbl_m_sides, #nvx_lbl_align, #nvx_lbl_font_size, #nvx_lbl_note').on('input change', updateLabelPreview);
			$('#nvx_chk_ttn, #nvx_chk_barcode, #nvx_chk_name, #nvx_chk_phone, #nvx_chk_address, #nvx_chk_order, #nvx_chk_items, #nvx_chk_total').on('change', updateLabelPreview);
			updateLabelPreview();
		}
	});
})(jQuery);
